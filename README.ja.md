...
- **Linux 起動サポート** - Steam ライブラリの取り込み、Linux/Steam ゲーム起動、Wine 経由の Windows 実行ファイル起動、`lunabox://` ショートカット登録に対応
...
- Windows インストーラーには [msys2](https://www.msys2.org/) と [NSIS](https://nsis.sourceforge.io/Main_Page) が必要
- Linux デスクトップビルドには、ディストリビューションに対応する Wails WebKitGTK/GTK 開発パッケージが必要
...
# スクリプトでローカルビルド（Linux）
./scripts/build.sh all 1.0.0-beta
```
