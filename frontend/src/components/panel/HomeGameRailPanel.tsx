...
export function HomeGameRailPanel({
  ...
  useEffect(() => {
    if (!isExpanded || activeTab !== "heatmap" || heatmapStats) {
      return;
    }
    void loadHeatmapStats();
  }, [activeTab, heatmapStats, isExpanded, loadHeatmapStats]);
  ...
  const tabOptions = useMemo(
    () => [
      ...
      {
        label: t("stats.heatmap.title"),
        value: "heatmap" as const,
        icon: <span className="i-mdi-calendar-month-outline text-base" />,
      },
    ],
    [t],
  );
      ...
      >
        <div className="h-48">
          {activeTab === "recent" ? (
            ...
            <div className="grid h-full grid-cols-4 items-center gap-4 px-8 py-2">
            ...
            <div className="flex h-full items-center justify-center px-8 py-1">
              {heatmapLoadFailed ? (
                <button
                  ...
                  className="mx-auto flex items-center gap-2 rounded-lg border border-brand-300/70 bg-white/20 px-4 py-2 text-sm font-medium text-brand-700 transition-colors hover:bg-white/35 data-glass:bg-white/10 data-glass:hover:bg-white/18 dark:border-white/15 dark:bg-black/20 dark:text-white/80 dark:hover:bg-black/30"
                >
