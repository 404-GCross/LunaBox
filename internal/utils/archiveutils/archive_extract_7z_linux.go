...
func resolveLinux7zz() (string, error) {
	...
	if toolPath, ok := resolveRepoLibFile("linux"+runtime.GOARCH, "7z", "7zz"); ok {
		return toolPath, nil
	}

	if toolPath, err := exec.LookPath("7zz"); err == nil {
		return toolPath, nil
	}
	if toolPath, err := exec.LookPath("7z"); err == nil {
		return toolPath, nil
	}

	return "", fmt.Errorf("bundled/system 7zz not found")
}
